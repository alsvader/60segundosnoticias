...
- **`release.yml`** (push a `main` o `workflow_dispatch`): reutiliza `ci.yml` como `ci-gates`, luego en paralelo `e2e-full` (Firefox/WebKit, los 8 journeys críticos), `visual` (regresión visual Linux), `docker-smoke` (imagen real), `lighthouse` — y solo si los cuatro pasan, `publish` (GHCR, tags inmutables por SHA + alias `-main`). Esta es la calificación FULL antes de que cualquier imagen sea públicamente consumible. A partir de ahí, dos jobs corren en paralelo (ninguno bloquea al otro): `provenance` (`needs: publish`, registra SHA/tags/timestamp en el step summary — nunca depende de una aprobación humana) y **`deploy`** (`needs: publish`, `environment: production` con *required reviewers* — pausa hasta aprobación manual, luego entrega el tag inmutable de este SHA a Dokploy vía `scripts/dokploy-deploy.ts` y corre `scripts/smoke-production.ts`; `openspec/changes/production-deployment-dokploy`). Un rollback a un SHA ya publicado usa el workflow separado `.github/workflows/rollback.yml` (mismo Environment, misma verificación de smoke, nunca reconstruye). Runbook operativo completo (cómo leer una falla de `deploy`, cómo ejecutar un rollback) en `docs/OPERATIONS.md`.
