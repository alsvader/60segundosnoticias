...
- **`release.yml`** (push a `main` o `workflow_dispatch`): reutiliza `ci.yml` como `ci-gates`, luego en paralelo `e2e-full` (Firefox/WebKit, los 8 journeys críticos), `visual` (regresión visual Linux), `docker-smoke` (imagen real), `lighthouse` — y solo si los cuatro pasan, `publish` (GHCR, tags inmutables por SHA + alias `-main`) y `provenance` (registra SHA/tags/timestamp en el step summary). Esta es la calificación FULL antes de que cualquier imagen sea públicamente consumible.
