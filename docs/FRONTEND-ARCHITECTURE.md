...
`src/app/(frontend)/layout.tsx` declara `export const dynamic = 'force-dynamic'` (Fase 5; removido en Fase 8 al verificar la invalidación por tags, restaurado en Fase 11 - `openspec/changes/runtime-public-rendering` - por una razón distinta: `next build` no debe requerir una base de datos alcanzable). Administrable sin rebuild (AC-NAV-004, AC-FOOT-001/002) ya no depende de esto: lo garantiza `unstable_cache` con revalidación por tags (ver "Cache" más abajo), independiente del modo de render de la ruta.
...
- **8 Home Blocks V1** (`src/payload/blocks/home/`): `EditorialIntro`, `HeroNews`, `CategoryExplorer`, `LatestPosts`, `PostsByCategory`, `FeaturedPosts`, `VideoFeature`. `Banner` se reubicó a `src/payload/blocks/shared/Banner.ts` — el mismo schema se usa desde `Home.layout` y `Pages.layout`, en vez de duplicarlo; `src/components/sections/banner-section.tsx` es el único `BannerSection`, fuera de `sections/home/`/`sections/pages/` precisamente porque ambas fases lo comparten. Es un CTA editorial interno (no publicidad): en Home (`layout="full-bleed"`, por defecto) la `<section>` lleva el fondo de la variante a ancho completo y el contenido va dentro de `Container`, igual que el resto de las Home Sections; en Pages (`layout="contained"`) el mismo contenido se presenta como tarjeta, porque la ruta de Pages ya envuelve todos sus blocks en un `Container`. La composición se deriva de la presencia de `image` (centrada sin imagen, dos columnas desde `lg` con imagen), sin campo de configuración (openspec `redesign-banner-cta`).
...
- **`RichText` de Page y `GalleryBlock`/`VideoBlock` de Page comparten
  ...
  `BannerSection` ya compartido desde Fase 6, con `layout="contained"`.
...
- **Cache**: cada función pública del DAL está envuelta en `unstable_cache` con tags específicos (`src/lib/cache/tags.ts`), invalidados con `revalidateTag(tag, { expire: 0 })` desde hooks `afterChange`/`afterDelete` (`src/lib/cache/invalidate.ts`, uno por Collection/Global en `src/payload/hooks/*/cache-invalidation.ts`). `force-dynamic` (Fase 5, removido en Fase 8, restaurado en Fase 11 por el invariante de build sin base de datos - ver arriba) no afecta esta capa: es independiente del Data Cache que gestiona `unstable_cache`. `getPostBySlug()`/`getCategoryBySlug()`/`getPageBySlug()` — las únicas consultas que resuelven por slug público, no por id — se tagean por slug, no por id, porque el id no se conoce hasta que la consulta resuelve.
